...
const VideoGridAdvanced = ({ 
  ...
  // Local video component that renders based on current layout
  const LocalVideoComponent = ({ className, containerClass = "", onClick = null }) => {
    const videoRef = useRef();
    
    useEffect(() => {
      if (videoRef.current && localVideoRef.current?.srcObject) {
        videoRef.current.srcObject = localVideoRef.current.srcObject;
      }
    }, [localVideoRef.current?.srcObject]);

    return (
      <div className={`relative group cursor-pointer ${containerClass}`} onClick={onClick}>
        <video
          ref={layout === 'grid' ? localVideoRef : videoRef}
          autoPlay
          muted
          playsInline
          className={className}
  ...
  // Sync video src object when switching layouts
  useEffect(() => {
  ...
  const renderSpeakerLayout = () => {
    const speakerPeer = speakerUserId && speakerUserId !== 'local' ? peers.get(speakerUserId) : null;
    ...
    <div className="fixed inset-0 bg-black z-40 flex items-center justify-center">
      <div className="relative w-full h-full flex items-center justify-center">
        ...
        <button
          ...
          className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-3 rounded-full transition-all duration-200 transform hover:scale-105"
        ...
        {/* Camera off overlay for theater mode */}
        {!isVideoEnabled && (
          <div className="absolute inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center">
            <div className="text-center text-white">
              ...
              <p className="text-2xl opacity-75">Câmera desligada</p>
            </div>
          </div>
        )}
      </div>
